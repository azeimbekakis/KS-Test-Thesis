...
test_autoarma <- function(x, pdist, qdist, B, working) {
...
working <- "auto"
counter <- 0
pvals_autoarma <- replicate(nrep, test_autoarma(x = genData(n, phi, theta, qdist), 
                                                   pdist = pdist, 
                                                   qdist = qdist, 
                                                   B = 1000, 
                                                   working = working)$p.value)
counter <- 0
pvals_naive <- replicate(nrep, test_standard_ks(genData(n, phi, theta, qdist),
                                                   pdist = pgamma,
                                                   shape = shape, 
                                                   scale = scale)$p.value)
...
# Save the histogram figure
ggsave(filename = "hist_autoarma.pdf", plot = hist_autoarma, path = "../Manuscript", height = 2.1, width = 6)
```

```{r Autoarma FD}
test_autoarma_FD <- function(x, pdist, qdist, B = 1000, working=c("auto", "2,2")) {
    z <- qnorm(pdist(x))
...
pvals_autoarma <- replicate(nrep, test_autoarma_FD(x = genData(n, phi, theta, qdist), 
                                                   pdist = pdist, 
                                                   qdist = qdist, 
                                                   B = 1000, 
                                                   working = working)$p.value)
...
# P-vals from using parametric bootstrap
pvals_parametric <- replicate(nrep, test_parametric_gamma(genData(n, phi, theta, qdist))$p.value)

pvals_naive <- replicate(nrep, test_super_naive_gamma(genData(n, phi, theta, qdist))$p.value)
...
# Generate histograms plot
hist_autoarma_FD <- ggplot(results) + 
                   geom_histogram(aes(x = p_values, fill = histogram), boundary = 0, binwidth = 1/20) + 
                   scale_x_continuous(name = "p-value", breaks=c(0, 1)) + 
                   theme(legend.position = "none") +
                   facet_wrap(~histogram, scales = "free_y")
...
# Save the histogram figure
ggsave(filename = "hist_autoarma_FD.pdf", plot = hist_autoarma_FD, path = "../Manuscript", height = 2.1, width = 6)
```
