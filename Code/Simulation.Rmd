...
# Naive function that incorrectly uses fitted parameters for gamma distribution
test_super_naive_autoarma <- function(x) {
...
working <- "auto"
counter <- 0
pvals_autoarma <- replicate(n_tests, test_autoarma(x = genData(n, phi, theta, qdist), pdist = pdist, qdist = qdist, B = 1000, working = working)$p.value)
counter <- 0
pvals_naive <- replicate(n_tests, test_super_naive_autoarma(genData(n, phi, theta, qdist))$p.value)

# Create a master data_frame with all the results
histogram <- c(rep("Autoarma", n_tests), rep("Naive", n_tests))
