...
\item
  ...
  $F_{\hat\theta_n}$
\item
  Fit $F_\theta$ to $X_1^*,...,X_n^*$ \eds{verify edit} and obtain estimate 
	$\hat\theta_n^*$ of $\theta$.
\item
...
\item
  ...
  \[
    D_n^* = \sqrt{n} \sup_x | F_n^* (x)- F_{\hat\theta_n}^*(x) |.
  \]
	\eds{please explicitly define $F_{\hat\theta_n}^*(x)$ here.  Or should it be
	$F_{{\hat{\theta}_n}^*}(x)$ instead? (Note placement of *)}
\item
...
\item
  Calculate bootstrap KS statistic
  \[
    D^* = \sup_x | \sqrt{n}\left(F_n^* (x)- F_{\hat\theta_n}^*(x)\right) - B_n(x) |.
  \]
	\eds{do we want an n subscript on $D^*$?  It is not consistently used below.}
  where $B_{n}(x) = \sqrt{n}(F_{n}(x) - F_{\hat\theta_n}(x))$ is the known
  bias term \citep{babu2004goodness}.
	\eds{same question about $F_{\hat\theta_n}^*(x)$ as above}
\item
  Repeat the previous steps a large number $B$ times and use the empirical
  distribution of $D^*$ to approximate the null distribution of the observed
  statistic. \eds{do we want an n subscript on $D^*$}
\end{enumerate}
The p-value of $D_n$ is, again, approximated by the proportion of the $D_n^*$
statistics that
are greater than or equal to~$D_n$. \eds{do we want an n subscript on $D^*$}
...
negative $\psi$ values led to conservative p-values. 
\eds{can p-values be liberal or conservative?  Not sure, but I think I have only
 heard of tests being described in this way} 
...
fitting process. The selected ARMA$(p, q)$ model with fitted
...
\item
  ...
  \]
	\eds{n subscript for $D^*$? Not consistently used.}
\item
  Repeat the previous steps a large number $B$ times and use the empirical
...
\end{enumerate}
The p-value of $D_n$ is, again, approximated by the proportion of the $D_n^*$
statistics that are greater than or equal to~$D_n$. \eds{n subscript for $D^*$?}
...
performance of the size of the KS test. The dependence structure determined by
the working ARMA process approximates a wide class of dependence structures.
