...
in~\eqref{eq:ks_standard} changes with the serially dependence.
...
would need specify a model for the dependence, whereas the primary interest
...
$Z_i = \Phi\{ F(X_i)\}$, $i = 1, \ldots, n$, where $\Phi$ is the distribution
...
\citep{hyndman2008automatic}. Since it is known that the mean of $Z_i$'s are
...
parameters will be used as the working model to generate bootstrap sample with
...
the true dependence is not covered the ARMA model, the working model may still
give a reasonable approximation that can
