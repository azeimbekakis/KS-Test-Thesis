...
\jy{how about removing the panels for $-0.1$ and 0.1?}
\eds{seems like too few if those are removed. How about $\{-0.4,-0.2,0.2,0.4\}$
or $\{-0.30,-0.15,0.15,0.30\}$? Is it easy/quick to generate the plots?} 
\jy{Will get jobs done for $\{-0.4,-0.2,0.2,0.4\}$}
...
there were more smaller p-values than expected, and 
hence, %conservative 
...
were more larger p-values than expected, and hence, 
%liberal 
conservative tests. 
\eds{the previous two sentences are not consistent with what is 
...
of this section).} \jy{Need to rephrase. A distribution function higher than
  another distribution means that it has more smaller values.}
\eds{do the edits fix the problem?}
\jy{can we rephrase ``than excepted'' to ``than there should be'' or something
  along the line?}
...
\item
  ...
  \]
  \eds{check placement of * in second F}
  \jy{the start should be with hat theta from step 3}
\item
  Repeat the previous steps a large number $B$ times and use the empirical
  distribution of the $B$ test statistics to approximate
  the null distribution of the observed statistic.
...
place of the known~$F$. \eds{check placement of * in second F}
...
Figure~\ref{fig:pp_ssf} shows the PP-plots resulting p-valuess of the
...
from the standard uniform distribution except in the MA(1) case.
\jy{needs an explanation ...}
\eds{if we had used a different MA parameter other than 0.8, would we see  
similar behavior?}  \jy{yes. I have more results for other MA coefficients to
  show when we meet.}
...
$\Gamma(8, 1)$. 
\eds{same AR and MA parameters as the previous section?} \jy{yes; edited.}
...
a procedure using bootstrap with the normal copula determined by an
automatically selected ARMA model to account for dependency shows positive
