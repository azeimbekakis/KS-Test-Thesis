...
p-value of the observed $D_n$, however, the null distribution is not the same as the
of $D_n$, however, is no longer the same as that in the standard
...
member distribution $F_{\hat\theta}$.
...
one would expect to see from a 1000 draws from the standard uniform
distribution.


To fix the problem, parametric bootstrap can be used to approximate the null
distribution of the testing statistic~$D_n$. For a large number~$B$, the
parametric bootstrap procedure is as follows.
\begin{enumerate}
\item
...
\item
  ...
  \]
\item
  Repeat the previous steps $B$ times and use the empirical
...
of the testing statistic with the bias correction from \citet{Babu}. The
