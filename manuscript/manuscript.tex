...
and sample size $n = 200$. The sample mean and sample standard
...
variance. This experiment was repeated 1000 times and the 
probability-probability plot (PP-plot) of the 1000 p-values are displayed in the 
top left plot of Figure~\ref{fig:pp_f}. If the test were
...
hypothesized distribution using the shape and scale \eds{maximum likelihood 
...
for all the $\psi$ values are displayed in Figure~\ref{fig:pp_s}. When
$\phi$ diverges from 0, the p-values do not look likely to be generated from a 
...
distributionfunction lower than the standard uniform distribution and, hence, 
liberal tests. \eds{check previous statement, not consistent with beginning of 
this section} The larger the magnitude of
$\psi$, the bigger the deviation from the desired level.
\eds{check that $\psi=.1$ plots are really from .1 and not 0}
