...
needed
\citep{babu2004goodness}. Details of the nonparametric bootstrap procedure are
...
coefficient $\psi \in \{-0.3, -0.1, 0.1, 0.3\}$, we generated 1000
...
distribution function higher than the standard uniform distribution function and, 
hence, conservative tests. Negative $\psi$ values led to p-values with a 
distribution function lower than the standard uniform distribution and, hence, 
liberal tests. \eds{the previous two sentences are not consistent with what is 
written at the beginning of this section. If I am interpreting the plots 
correctly, then positive $\psi$ leads to p-values that are too low which would
correspond to liberal tests (consistent with what was written at the beginning 
of this section).} 
...
The semiparametric bootstrap procedure was evaluated in simulation. We 
considered three true dependence structures on the standard normal scale 
(ARMA(1, 1), AR(2), and MA(1)) and two marginal distributions 
($N(8,8)$ and $\Gamma(8,1)$).  For each dependence structure and marginal distribution combination, we generated 1000 series of length 200.  For each series, we applied the semiparametric bootstrap procedure with serial
...
Figure~\ref{fig:pp_ssf} shows the results of the
procedure performed on data generated with dependence structures of ARMA(1, 1),
AR(2), and MA(1). Naively fitting parameters and not adjusting for dependence  clearly deviates from a uniform distribution of p-values. 
Parametric bootstrap provides some correction but does not account for 
dependence, 
\eds{just want to verify: you are using parametric bootstrap from Section 2 for
the top portion of Figure~\ref{fig:pp_ssf}? (Not the totally naive procedure 
that ignores both issues)} 
so therefore the results of the copula remedy are more accurate and favorable.
...
solution. Regardless of the true dependence in the data we assume an ARMA$(p,q)$
dependence structure. As long as this assumption is a close approximation, the
correction should still provide a reasonable approximation that can be useful 
for practical purposes. 
%Regardless of the true dependence in the data we assume an AR(1)
...
parameters are accounted for \eds{i.e., procedure from Section 2?  It may be 
less confusing if we had a name for the procedure discussed in each section 
rather than using ``naive'' to describe both levels of ignorance?}, appear to 
have higher power than the proposed tests, but this is not reliable since they 
do not hold their sizes as demonstrated in the Figures. 
...
is used \citep{babu2004goodness}. In the case of serially dependent data, 
a procedure using bootstrap with copulas to model dependency shows positive
results. When both assumptions are violated, i.e., where the data has serial 
dependence and parameters must be fitted, an adjusted copula procedure also 
shows positive results. The tests appear effective for a variety of families of 
distributions. The copula remedy is not a complete solution and has limitations. 
Regardless of the true dependence, we assume an ARMA($p,q$) dependence structure. 
%Therefore, if the AR(1) dependence structure is a 
%close approximation of the truth, the approach can work.
%However, in cases such as AR(2), if 
%the approximation is too far from the true
%dependence structure the approach does not completely remedy the issue.
Therefore, if the assumed dependence structure is a close approximation of the
truth, the approach can work. However, if the approximation is too far from the 
true dependence structure the approach does not completely remedy the issue. 
As well, tests were only performed with the normal copula. It is possible that 
other copulas could provide stronger results based on the true dependence of the 
data.
