...
Despite its popularity, the test is frequently in the literature and
...
-conservative outcome when estimating such parameters. 
...
relevant literature would be ``misusing'' the KS test, there is still a
...
an ivory tower tend to be doubt that anyone would be misusing the KS test as we
...
autoregressive sieve, i.e., autore-gressive models the order of which increases
simultaneously with the sample size at a suitably slower rate than the sample
size \citep[e.g.,][]{psaradakis2017distance, psaradakis2020normality}.
